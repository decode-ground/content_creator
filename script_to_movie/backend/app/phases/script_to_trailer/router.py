...
async def analyze_endpoint(project_id: int, db: AsyncSession = Depends(get_db)):
    project = await _get_project(project_id, db)
    scenes = await analyze_script(db, project)
    return {"success": True, "scenesCount": len(scenes)}
